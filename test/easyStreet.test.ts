...
test("no par on the three costs 0.3", () => {
...
test("an eagle is still one hole", () => {
  ...
  assert.equal(score(street(-2, -2, 1)).strokes, -0.3, "two eagles are two, not four");
});
...
test("it reads gross, so handicap strokes do not create pars", () => {
  ...
  // three IS a net par on all three — and Easy Street still charges him 0.3.
  ...
  assert.equal(r.contests.easyStreet!.strokes, 0.3, "and gross bogey pays the penalty anyway");
});
...
test("a picked-up hole is played, and is not a par", () => {
  ...
  assert.equal(r.strokes, -0.3, "12 and 13 still carry it");
  assert.equal(r.detail, "2 of 3 at par or better");
});
