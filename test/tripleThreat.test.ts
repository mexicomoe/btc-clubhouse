 ...
 *
 *   gross triple bogey or worse        +0.2
 *   net par or better on the next hole −0.4
 *
...
test("the rates are one flat pair, with no bands", () => {
  assert.deepEqual(DEFAULT_CONTESTS.tripleThreat, { perTriple: 0.2, perRecovery: -0.4 });
});
...
test("a triple not answered costs 0.2", () => {
...
test("worse than a triple is still one triple", () => {
  for (const over of [3, 4, 5, 8]) {
    ...
    assert.equal(r.strokes, 0.2, "par + " + over);
  }
});
...
test("answering a triple with a net par turns 0.2 into −0.2", () => {
...
test("better than a net par answers it just as well", () => {
  assert.equal(score(card((g) => { at(g, 1, 3); at(g, 2, -1); })).strokes, -0.2);
  assert.equal(score(card((g) => { at(g, 1, 3); at(g, 2, -2); })).strokes, -0.2);
});
...
test("the recovery is read on net, not gross", () => {
  ...
  assert.equal(score(c).strokes, -0.2, "gross bogey on 2 is a net par");
  const scratch = card((g) => { at(g, 1, 3); at(g, 2, 1); }, 0);
  assert.equal(score(scratch).strokes, 0.2, "off scratch the same card does not answer");
});
...
test("consecutive triples leave the first unanswered", () => {
  ...
  assert.equal(r.strokes, 0.0, "0.4 charged, 0.4 given back");
});
...
test("a pick-up does not answer the triple before it", () => {
  ...
  assert.equal(r.strokes, 0.2, "the triple on 1 stands unanswered");
});
...
test("a hole not played is neither a triple nor an answer", () => {
  ...
  assert.equal(blank.strokes, 0.2, "hole 2 is a gap, so nothing answers the triple");
...
// 0.2 and −0.4 do not add exactly in binary: three triples answered twice is
// 0.6000000000000001 − 0.8 without rounding. Every figure the contest returns
