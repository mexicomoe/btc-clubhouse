 ...
 *
 *   gross triple bogey or worse        +0.3
 *   net par or better on the next hole −0.6
 *
...
test("the rates are one flat pair, with no bands", () => {
  assert.deepEqual(DEFAULT_CONTESTS.tripleThreat, { perTriple: 0.3, perRecovery: -0.6 });
});
...
test("a triple not answered costs 0.3", () => {
...
test("worse than a triple is still one triple", () => {
  for (const over of [3, 4, 5, 8]) {
    ...
    assert.equal(r.strokes, 0.3, "par + " + over);
  }
});
...
test("triples add up", () => {
  ...
  assert.equal(r.strokes, 0.9);
  assert.equal(r.detail, "3 triples, 0 answered");
});

/* ---- the recovery ---- */

test("answering a triple with a net par turns 0.3 into −0.3", () => {
...
test("better than a net par answers it just as well", () => {
  assert.equal(score(card((g) => { at(g, 1, 3); at(g, 2, -1); })).strokes, -0.3);
  assert.equal(score(card((g) => { at(g, 1, 3); at(g, 2, -2); })).strokes, -0.3);
});
...
test("the recovery is read on net, not gross", () => {
  ...
  assert.equal(score(c).strokes, -0.3, "gross bogey on 2 is a net par");
  const scratch = card((g) => { at(g, 1, 3); at(g, 2, 1); }, 0);
  assert.equal(score(scratch).strokes, 0.3, "off scratch the same card does not answer");
});
...
test("consecutive triples leave the first unanswered", () => {
  ...
  assert.equal(r.strokes, 0.0, "0.6 charged, 0.6 given back");
});
...
test("a pick-up does not answer the triple before it", () => {
  ...
  assert.equal(r.strokes, 0.3, "the triple on 1 stands unanswered");
});
...
test("a hole not played is neither a triple nor an answer", () => {
  ...
  assert.equal(blank.strokes, 0.3, "hole 2 is a gap, so nothing answers the triple");
...
// 0.3 and −0.6 do not add exactly in binary: three triples answered twice is
// 0.8999999999999999 − 1.2 without rounding. Every figure the contest returns
