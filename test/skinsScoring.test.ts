 ...
 *
 * A skin is worth `fairShare × groups / 18`: −0.06 over two groups, −0.11 over
 * four, −0.17 over six, −0.33 over twelve. That is the value at which an even
 * share of the eighteen comes to 0.5 whatever the size of the field. The total
 * is capped at 1.5, because the winning group's haul does
...
test("a skin is worth what makes an even share come to the fair share", () => {
  ...
  assert.equal(config.fairShare, -0.5);

  const per = (g: number) => Number(skinValue(config, g).toFixed(2));
  assert.equal(per(2), -0.06, "two groups");
  assert.equal(per(4), -0.11, "four groups");
  assert.equal(per(6), -0.17, "six groups");
  assert.equal(per(12), -0.33, "twelve groups");
});

// This is the property the value exists to have, and the reason it is not
// `fairShare / groups`: that made a fair share worth 2.25 over two groups and
// 0.06 over twelve, which is the opposite of flat.
test("an even share of the eighteen is worth the same in any field", () => {
  ...
  for (const groups of [2, 3, 6, 9, 18]) {
    ...
    assert.ok(Math.abs(paid - 0.5) < 0.05,
      `${groups} groups: an even share of ${share} paid ${paid.toFixed(2)}, not 0.5`);
  }
});
...
test("a skin is worth to the hundredth exactly what the tab prints", () => {
  ...
  // 0.0556 would print as 0.06 and pay as though it were not.
  const config = DEFAULT_CONTESTS.skins!;
...
test("the ceiling is 1.5", () => {
  const config = DEFAULT_CONTESTS.skins!;
  assert.equal(config.maxSkinStrokes, -1.5);

  assert.equal(skinStrokes(18, config, 4), -1.5, "a four-group rout reaches it");
  assert.ok(skinStrokes(13, config, 4) > -1.5, "but thirteen skins does not");
  assert.equal(skinStrokes(18, config, 12), -1.5, "and a big field cannot pass it");
  assert.equal(skinStrokes(5, config, 12), -1.5, "which at twelve groups is soon reached");
});
...
test("skins are added to the final and shown as a contest", () => {
  ...
  // Two groups, so a skin is worth −0.06 and eighteen of them −1.1 — inside the
  // ceiling, because in a small field each skin is worth little.
  assert.equal(by["Strong"].contests.skins!.strokes, -1.1);
...
for (const [carts, eighteen, capped] of
     [[2, -1.1, false], [4, -1.5, true], [6, -1.5, true]] as const) {
...
test("a two-group field is decided rather than levelled", () => {
  ...
  // Nine of eighteen over two groups is an even share, so each is paid about the
  // 0.5 the fair share is worth — the property the value is built to have.
  assert.ok(level.every((r) => r.contests.skins!.strokes === -0.5), "and both paid the same");
